...
app.post('/process', upload.single('video'), (req, res) => {
  try {
    ...
    // 🟩 安定版コマンド（元通りの構成）
    if (mode === 'vhs') {
      ffmpegCommand = `"${ffmpegPath}" -y -i "${inputPath}" -i "./overlay/rec.png" -filter_complex "[0:v]scale=iw/2:ih/2,noise=alls=5:allf=t,chromashift=cbx=2:cby=0:crx=-2:cry=0[v];[v][1:v]overlay=20:20" -b:v 400k -c:v libx264 -pix_fmt yuv420p -preset fast -c:a aac -b:a 64k "${outputPath}"`;
    } else if (mode === 'pixel') {
